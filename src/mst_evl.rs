...
impl EvoMonster {
    ...
    fn evolvable(&self, mst: &Monster) -> bool {
        self.min_attack <= mst.attack
            && mst.attack <= self.max_attack
            && self.min_defenece <= mst.defence
            && mst.defence <= self.max_defence
            && self.min_speed <= mst.speed
            && mst.speed <= self.max_speed
    }
}
...
mod tests {
    ...
    fn test_solve0() {
        ...
        evos.push(super::EvoMonster::new(
            "paizabird",
            100,
            200,
            130,
